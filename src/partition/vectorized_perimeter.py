...
def compute_perimeter_hessian_sparse(pa: PartitionArrays) -> np.ndarray:
    ...
    # Phase A: vectorised accumulation via pre-computed offset arrays.
    # Multiple segments may share the same Hessian entry, so np.add.at
    # is required (it correctly handles duplicate indices).  The else
    # branch is the legacy per-row loop, kept for backwards compatibility
    # with snapshots compiled before Phase A and used by the regression
    # test in testing/test_phase_a_vectorised_hessian.py.
