...
def compute_area_hessian_sparse(pa: PartitionArrays,
    ...
    if np.any(m1):
        ...
        # Phase A: vectorised accumulation.  The pre-computed offsets and
        # cell_active mask were built from the same np.flatnonzero(m1)
        # ordering used here, so btri_cell[m1][active] aligns row-for-row
        # with btri1_hess_off_*.  See plan §3.4.
        if pa.btri1_hess_off_aa is not None:
    ...
    if np.any(m2):
        ...
        # Phase A: vectorised accumulation (mirrors the m1 block above).
        if pa.btri2_hess_off_aa is not None:
