...
# accepted/gtnorm). Must exceed every refine_*_patience so the trigger windows
# are unaffected; patiences are O(10), so this is ~250x margin while keeping the
# logs O(1) in memory instead of growing with iteration count.
...
class ProjectedGradientOptimizer:
	...
	def __init__(
		...
		# Logging cache. Bounded in memory: 'iterations' is a running count (its
		# only reader wants the total-iteration count), and the trigger reads
		# only the last refine_*_patience entries of the scalar traces, so capped
		# deques are behaviorally identical to unbounded lists. The old
	...
	def optimize(
		...
		with open(summary_filename, 'w') as summary_fh, h5py.File(internal_data_filename, 'w') as h5f:
			...
			if self.pgd_reduced_gradient:
				self.log.setdefault('accepted', deque(maxlen=_LOG_WINDOW_CAP))
				self.log.setdefault('gtnorm', deque(maxlen=_LOG_WINDOW_CAP))
			...
			for k in range(maxiter):
				...
				if enable_refinement_triggers and (k + 1 >= self.refine_patience):
					if self.pgd_reduced_gradient:
						...
						recent_acc = list(self.log['accepted'])[-self.refine_patience:]
						...
						if stalled:
							...
							if stable:
								...
								if not gt_ok and len(self.log['gtnorm']) >= refine_gnorm_patience:
									recent_gt = list(self.log['gtnorm'])[-refine_gnorm_patience:]
