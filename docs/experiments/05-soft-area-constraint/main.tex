...
against the control's \(+0.13\%\).\footnote{\textbf{Reproduction note
(2026-08-15).} \code{BalancedReadoutConfig.max\_repair\_sweeps} was raised from
\(50\) to \(200\) after these numbers were measured, so the readout quoted here
no longer reproduces from a bare
\code{scripts/balanced\_readout.py --solution …} --- it was truncated at the cap,
and the higher ceiling lets it keep going. Reproduce it exactly with
\code{--max-repair-sweeps 50}. The cap is a ceiling and never a schedule (the
sweep loop breaks early once no candidate move improves), so any readout that
converged in under 50 sweeps is unaffected.} Phase~2 inherits that geometry and
