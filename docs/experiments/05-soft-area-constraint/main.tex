...
\textbf{One narrower variant remains open} at the time of writing (exact
treatment at level~0, soft from level~1); \S\ref{sec:hybrid} states what it does
and does not escape.
\end{tcolorbox}
...
Its prognosis is nonetheless poor, and for a reason this report already
establishes: levels 1--4 keep the Euclidean projection and therefore defect~(2),
so they should collapse after 150--250 iterations each and do almost no
refinement. The measurement is cheap (\(\approx\)2~h) and is being run to convert
that prediction into a number.
