...
partitions at \(N{=}400\) and \(N{=}500\) from \(920\,\mathrm{s}\) and
\(1{,}322\,\mathrm{s}\), agreeing with \(\sqrt{N}\) scaling on a fixed mesh to
better than \(0.06\%\) across three points --- while on the \emph{same} mesh the
...
Exploratory runs & \code{results/arm\_mbo\_20260820\_133709\_npart400\_V114144\_seed84172851}
  and \code{results/arm\_mbo\_20260822\_120646\_npart500\_V114144\_seed84172851}
  --- \(N{=}400\) and \(N{=}500\), \textbf{no anchor}, and none possible
  (\S\ref{sec:n400}). Scored against nothing; reported for gates, cost and
  absolute perimeter. The \(N{=}500\) Phase~2 campaign was run with
  \code{-{}-profile}. \(N{=}500\) predates the arm \code{experiment.yaml}
...
\section{Result 5 --- \(N{=}400\) and \(N{=}500\): existence where the anchor cannot exist}
\label{sec:n400}
...
\(N{=}400\) and \(N{=}500\) are past that point, and this section reports them as
what they are: \textbf{existence-and-validity} results with no anchor, no margin,
and no comparison of perimeters. The runs are
\code{arm\_mbo\_20260820\_133709\_npart400\_V114144\_seed84172851} and
\code{arm\_mbo\_20260822\_120646\_npart500\_V114144\_seed84172851}, both executed
in the driver's exploratory \code{-{}-config} mode, where the mesh-match assertion
is skipped because there is nothing to match. \(N{=}500\) deliberately reuses
\(N{=}400\)'s ladder unchanged, which converts the \(\sqrt{N}\) argument below
from a single check into a three-point one.

\subsection*{The results}

Both ladders run to \(V = 114{,}144\) in five levels. Phase~1 replacement wall is
\(\mathbf{920\,\mathrm{s}}\) (15.3 min) at \(N{=}400\) and
\(\mathbf{1{,}322\,\mathrm{s}}\) (22.0 min) at \(N{=}500\). All three validity
gates pass on the \emph{raw} labels at both, re-verified with
\codefile{testing/check\_fragmentation.py} independently of \code{arm\_report.yaml}:

\begin{center}\small
\begin{tabular}{@{}llll@{}}
\toprule
gate & \(N{=}400\) & \(N{=}500\) & note \\
\midrule
dormant & 0 dead, 0 weak & 0 dead, 0 weak & vacuous for arms (one-hot) \\
area imbalance & 0, worst \(\mathbf{0.7773\%}\) & 0, worst \(\mathbf{1.0993\%}\) & bars \(1.1214\%\) / \(1.4017\%\) \\
connectivity & \(\mathbf{0}\) fragmented & \(\mathbf{0}\) fragmented & \emph{the only gate with content} \\
\bottomrule
\end{tabular}
\end{center}

Phase~2 reaches \(\mathbf{368.6603}\) (iterate 19 of 20, 29{,}287 variable points,
max equal-area violation \(8.18\times10^{-7}\)) and \(\mathbf{412.1138}\)
(iterate 10 of 20, 32{,}704 variable points, \(1.10\times10^{-6}\)) --- both
converged to tolerance, so the partitions are genuinely equal-area rather than
approximately so. Note the \(N{=}500\) best is \emph{mid-trajectory}, so it is not
censored: the run descends to 412.1138 by iterate 10 and then oscillates up to
\(\approx412.14\), the migration-cycling plateau. Both deliverables are exported
and \code{finalised=True}, at
\code{partition/torus\_partition\_n400\_s84172851.h5} and
\code{...n500\_s84172851.h5}.
...
Phase~2 took \(\mathbf{7{,}326\,\mathrm{s}}\) at \(N{=}400\) and
\(\mathbf{8{,}829\,\mathrm{s}}\) (2.45\,h) at \(N{=}500\), against Phase~1's
920\,s and 1{,}322\,s. The stage this report replaces is therefore
\(\mathbf{11\%}\) and \(\mathbf{13\%}\) of end-to-end cost. The
...
The \(N{=}500\) campaign was run with \code{-{}-profile}, and the breakdown is
not what the forward plan assumed. \textbf{The IPOPT solver accounts for
\(459\,\mathrm{s}\), or \(5.2\%\) of the campaign} --- 4{,}023 solver iterations
across the 20 topology iterations, i.e.\ 201 per topology iteration, so it hits
\code{max\_opt\_iter: 200} every time and reaching that cap costs only 23\,s.
Roughly \(95\%\) of Phase~2 is spent \emph{outside} \code{optimize()}: contour
