...
Three points cannot fit a law, and the two exponents are not equal, so the
defensible statement is \emph{superlinear and steepening}, not ``quadratic''.
Either way it is the binding constraint on \(N\): at this trend \(N{=}1000\)
implies a Phase~2 of roughly \(7\text{--}9\,\mathrm{h}\), against a Phase~1 that
remains under an hour.
