...
secondary description. The two exceptions are marked.
...
present kernel belongs to (\S\ref{sec:kernel}). Alberti and Bellettini's nonlocal phase-transition model is the underlying
theory; the held Part~I \cite{alberti1998nonlocal} is the optimal-profile paper,
and the $\Gamma$-limit itself is in its companion (European J.\ Appl.\ Math.\ 9,
1998), which is not held.

\paragraph{Volume constraints.} Ruuth and Wetton \cite{ruuth2003volume} enforce a
prescribed volume in the two-phase scheme by shifting the threshold level away
from $\tfrac12$ until the volume is met. \emph{(Cited second-hand: the paper is
paywalled; its content is taken from the descriptions in \cite[\S2]{jacobs2018auction}
and \cite{laux2017bulk}, which proves its convergence, and the bibliographic
details from the publisher's listing.)} Jacobs, Merkurjev and Esedo\u{g}lu
