...
The method of Bogosel and Oudet \cite{bogosel2017partitions} represents a
...
assignment of each mesh vertex to one cell. This step is \textbf{prescribed by the
source paper itself}, whose equation~(5--1) defines
\[
  \phi_i(x) \;=\;
  \begin{cases}
    1 & \text{if } u_i(x) \ge \max_{j \neq i} u_j(x),\\
    0 & \text{otherwise,}
  \end{cases}
\]
i.e.\ pointwise \emph{winner-take-all}. We refer to this conversion as the
\emph{readout}, by analogy with instrumentation, where the internal state of a
device and the value it reports are distinct objects and the conversion between
them is a separate source of error.

\begin{remark}[What the source paper does and does not say about its own readout]
\label{rem:what-paper-says}
Read directly (\codefile{docs/papers/Partitions of Minimal Length on Manifolds.pdf}),
\cite{bogosel2017partitions} \emph{does} discuss the readout and names two
difficulties with it: a small void around each triple point, ``included in one of
the triangles of the mesh, and can be dealt with''; and contours that ``pass
through the middle of the edges'', producing zigzagged boundaries ``whose length is
significantly larger than the optimal total perimeter''. The second is the stated
motivation for the direct constrained optimization that follows --- our Phase~2 ---
which re-imposes equal area \emph{on the discrete cells}.

\textbf{Neither named difficulty is the one analysed here.} The paper's reported
``Area tol.'' figures, $2$--$5\times10^{-7}$, are the residual of the
\emph{continuous} constraint $\langle v, \phi_i\rangle = A/n$ --- the same
constraint we use --- and not the deviation of the \emph{extracted} territory. That
latter quantity is what \eqref{eq:gap} below computes, and it does not appear in
the paper.

\textbf{The reason is scale, and it is entirely reasonable.} The paper demonstrates
$n \in \{2,\dots,24,32\}$ on the sphere, $n \in \{2,4,6\}$ on the double torus, and
--- on the torus with exactly our parameters $R=1$, $r=0.6$ --- $n \in [2,11]$. This
project works at $N = 100$ to $400$, i.e.\ \textbf{roughly $10$ to $36\times$ beyond
the largest case the method was demonstrated on for this surface.} By
\eqref{eq:bandscaling} the readout error grows like $\sqrt{N/V}$, so at $n \le 11$
it is small enough that the subsequent constrained optimization absorbs it, and the
question never arises. The contribution here is therefore a \emph{scaling} result,
not the discovery of an oversight.

One further observation, recorded neutrally: the paper notes that a cell's discrete
contour ``may have one or more connected components''. That is a statement about the
contour data structure accommodating several loops, not an analysis of cells
fragmenting as a failure mode, and it should not be read as anticipating
\S\ref{sec:artifact2}.
\end{remark}
...
question. Nothing here contradicts \cite{bogosel2017partitions}: its readout is
well suited to the range it was demonstrated on, and what follows quantifies an
error term that only becomes binding an order of magnitude beyond that range.
\end{remark}
...
\subsection{The idea, before the algebra}

\begin{tcolorbox}[colback=olive!4!white, colframe=olive!55!black,
                  title={Intuition: one handicap per cell}, fontupper=\small]
The failure of \S\ref{sec:artifact1} is a cell that is \emph{everywhere second
place}. Its density is spread as a wide, low halo: the total is exactly right, but
at no vertex is it the largest, so it wins almost nothing.

The remedy is to stop comparing the raw densities. Give each cell a single number
of its own --- a handicap --- added to its density \emph{everywhere}, and compare
the adjusted values instead. Raise cell $k$'s handicap and it begins to win
vertices it had been narrowly losing.

The essential detail is the \emph{order} in which it wins them. The first vertices
to flip are those where $k$ was losing by the smallest margin, and for a density
that is largest near the cell's own core those are the vertices just outside the
territory it already holds. The cell therefore grows outward from where it is
already strong, rather than acquiring territory wherever the arithmetic happens to
put it. That is the property this whole construction is chosen for; it is made
precise in Proposition~\ref{prop:nested}.

That leaves one question, which the rest of this section answers: \emph{how does
one choose the $N$ handicaps so that every cell ends up with exactly its target
area?}
\end{tcolorbox}
...
\begin{tcolorbox}[colback=olive!4!white, colframe=olive!55!black,
                  title={Intuition: why this is a \emph{transport} problem},
                  fontupper=\small]
The classical transport problem is literally about moving goods: there are
supplies in some places, demands in others, and a cost per unit moved from each
supply to each demand; one seeks the cheapest matching. It dates to Monge and was
put into its modern form by Kantorovich \cite{villani2003topics}.

Here the \emph{supplies} are the mesh vertices, each carrying its piece of surface
area $v_i$. The \emph{demands} are the $N$ cells, each requiring exactly $\Atgt$.
The \emph{cost} of giving vertex $i$ to cell $k$ is how poor a fit that is, and
with $s_{ik} = \log u_{ik}$ the cost $-s_{ik}$ is small exactly where cell $k$'s
density is large. So ``assign every vertex somewhere, give every cell its target
area, and prefer assignments the relaxation already likes'' is a transport
problem, and nothing more exotic is needed to state it.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=olive!4!white, colframe=olive!55!black,
                  title={Intuition: the dual is a pricing problem},
                  fontupper=\small]
Every minimisation of this kind has a partner problem --- its \emph{dual} --- that
looks quite different but has the same answer. For transport, the dual is a
question about \textbf{prices}, and that is where the intuition lives.

Instead of planning the assignment centrally, let each cell post a
\emph{subsidy}. Every vertex then independently takes whichever cell offers it the
best deal: strongest density \emph{plus} that cell's subsidy. The dual problem is
to \textbf{find the subsidies such that, when every vertex acts purely in its own
interest, the resulting demand comes out exactly right} --- each cell receiving
precisely $\Atgt$.

Those subsidies are the handicaps of \S6.1, and $\psi$ below is exactly them. Two
consequences are worth drawing out explicitly, because the algebra states them
without remarking on them.

\textbf{Why $N$ numbers are enough.} The dual has one unknown per \emph{cell},
not one per vertex --- $N$ of them, whether the mesh carries ten thousand vertices
or a hundred thousand. That is the structural reason a correction this small can
fix the readout at all, and it is why the cost of the correction is seconds rather
than comparable to the relaxation it repairs.

\textbf{Why a naive iteration finds them.} $D$ below is convex: bowl-shaped, with
no false bottoms to be trapped in. So the obvious rule --- raise the subsidy of any
cell that came out too small, lower it for any that came out too large, repeat ---
is not a heuristic that happens to work, but descent on that bowl. That is the
content of Proposition~\ref{prop:subgrad}.

The economic reading is not merely decorative. Adjusting prices until every
bidder is satisfied and every item allocated is precisely Bertsekas' \emph{auction}
algorithm \cite{bertsekas1988auction}, and the same idea drives the
volume-constrained scheme of \cite{jacobs2018auction}. What follows is that idea
in its convex-duality form.
\end{tcolorbox}
...
\textbf{convex} and piecewise linear --- the bowl of the preceding box, with flat
facets and no local minimum that is not global. Complementary slackness forces every vertex
...
In plain terms: a vertex is indivisible --- one cannot give a cell 40\% of one ---
so balance can only ever be adjusted in steps of about one vertex's area, and a
target that falls between two steps cannot be hit exactly. Consequently the exact
optimum of \eqref{eq:lp} is in general \emph{fractional}, the readout
\eqref{eq:shifted} is a rounding of it, and rounding the $\le N-1$
...
connectivity: a single number per cell can say \emph{how much} that cell wins, and
nothing at all about whether what it wins is in one piece. Nor is this hypothetical
--- Remark~\ref{rem:shifts-fragment} records the offsets doubling the component
count of two cells on the run whose figures appear in \S\ref{sec:measured}. A
second pass therefore restores connectivity. It operates on labels only.
