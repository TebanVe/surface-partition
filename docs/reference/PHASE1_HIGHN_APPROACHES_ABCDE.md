...
| **B** | Volume-constrained multiphase threshold dynamics (auction-dynamics MBO) | 2 | ✅ **BUILT AND MEASURED (2026-08-19)** — `src/partition/mbo_auction.py`. Succeeds on both anchors: N=100 −0.455% / −0.590% at 211–225×, N=300 −1.044% at 318.8×. **0 FRAGMENTED at both N on RAW labels, no readout needed** — connectivity being the only gate with content for B (dormant is vacuous on one-hot labels; area is what B optimizes). Raw PGD gives 2 fragmented at N=300. **Demonstrated to N=400, N=500 and N=750** (2026-08-20/23): valid + exported in 920 s / 1,322 s / 3,614 s, where PGD on the *same* V=114,144 mesh needed 206,344 s at N=300 and still failed two gates — so past N=300 the claim is existence-and-validity, with no anchor and none possible. √N scaling holds across N=100/400/500 on that fixed mesh to better than 0.06%. N=750 and N=1000 use **re-based ladders** (bases 162×154 and 224×212) because the standard coarsest rung falls *below* the τ freeze threshold there. Both were **also** re-run on the standard ladder for mesh-matched downstream use, deliberately entering that sub-threshold regime: both still pass all three gates, the frozen rung costing ~2/3 of its own work and manufacturing fragmented cells that the ladder then heals completely — the distinction from PGD, where the same insult is permanent. Phase 2, not Phase 1, is now the binding cost. See `docs/plans/PHASE1_BC_REPLACEMENT_PLAN.md` Phase A |
