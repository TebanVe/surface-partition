...
├── test_balanced_assignment_solver.py   # Phase 0a gates: (1) approach A byte-identical after generalizing the solver; (2) foreign score scales (B's diffused y, C's -d^2) converge on the arms' ITERATED state. Bar = max(2x granularity, 1.25x strong-ref, A's own home-scale dual stall)
