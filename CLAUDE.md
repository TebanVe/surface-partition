...
pre-registered falsifiers anchored on the N=100 deliverable. Read it together
with `docs/reference/PHASE1_HIGHN_APPROACHES_ABCDE.md`. It records a verified
finding that blocks any N=300 comparison: **no N=300 run has ever had Phase 2
run on it**, and neither λ=11.5 run completed (`run_20260813_000958` has no
solution file; `run_20260813_003231` stops at `checkpoint_level03.h5`), so the
only usable N=300 solutions are the λ=12 pair at V=47,488.
