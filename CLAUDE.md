...
├── h5util.py                     # `create_dataset()`: transparent gzip for the large one-hot payloads. The bulk of this project's disk footprint was a few datasets storing a HARD 0/1 field as uncompressed float64 — Phase 1's `x_opt`/`x0`, Phase 2's `indicator_functions`. Measured: gzip=4 shrinks such a block **459×** (457 MB → 1.0 MB) and is *faster* end to end, because writing 457 MB of mostly-zeros costs more I/O than compressing costs CPU. ⚠ **Applied ONLY to one-hot payloads** — `arm_harness` (`x_opt`/`x0`) and `pipeline_orchestrator` (`indicator_functions`). The ratio comes entirely from the data being almost all zeros and does **not** generalise: measured on Phase 1's *continuous* PGD density (10.1M distinct values) the same setting gives **1.21×** and makes the write **42.6× slower** (0.04 s → 1.64 s on 91 MB). So `relaxation.py` and `balanced_readout.py` deliberately do **not** use it — and relaxation's write is the per-level checkpoint that makes multi-day runs survivable, not a path to slow 40× for a 1.2× saving. Datasets under 1 MiB and scalars are left alone. **gzip is a standard HDF5 filter so h5py decompresses transparently: no reader changed, and pre-compression files still open.** NOT applied to `src/export/writer.py` (the format a downstream repo consumes — not ours to change unilaterally) or to PGD traces (continuous data, only 1.2× compressible)
...
├── torus_{A1,A2,A3,A4,B1,B2,B3,B4,C1}_n*.yaml  # ★ The **downstream Monte-Carlo study set** (9 configs, committed 24e73c2). Group A varies surface SIZE at fixed shape (R/r=1.667) to separate global mesh size from local density; group B varies SHAPE at constant area (Rr=0.600); C1 brackets the perimeter minimum from below at n=25. Every ladder lands within +0.13% of the requested V and hits the requested verts/cell exactly. ⚠ **All of them run their fine levels BELOW the τ freeze ratio of 1.0** (as low as 0.524) — expected, because the ratio falls as cells grow relative to the mesh, and high verts/cell is the *safe* regime. **RUN SO FAR: C1** (worst 0.0217%, bar 0.0701%, 0 fragmented, 99 s, Phase 2 **92.5422**) and **B1** (worst 0.0485%, bar 0.1502%, 0 fragmented, 141 s, Phase 2 **131.5610**) — both clean at ratios 0.524 and 0.614, confirming the argument. Run with `--out-root results/mc_study` to keep the study in its own tree
...
├── 09-balanced-readout/  ← **the readout correction that is actually shipped.** Exact territory–mass identity `T_k − Ā = gain_k − lost_k` (both terms supported only on the diffuse band); band fraction `f_b ~ √(N/V)`, so the error is governed by **verts-per-cell alone**; why disconnection is *admissible* (argmax super-level sets need not be connected); the transportation-LP dual whose subgradient **is** the implemented ψ update; nested monotone growth = the locality property, with what it does *not* give; and **non-integrality** ⇒ exact balance is generically unattainable (real lumped masses, so the LP optimum is fractional, splitting ≤ N−1 vertices). ⚠ The vertex mass `max vᵢ/Ā` is a **scale/quantum for calibrating bars, NOT a lower bound** — an earlier draft claimed it was a floor and that was false (counterexample in the text, achievable deviation 7 orders of magnitude below the claimed floor). ⚠ The dual shifts **can worsen connectivity en route**: in `run_20260806_123326` the two fragmented cells go 2→4 components after the shifts, before repair removes them — so the repair stage is necessary, not cosmetic. Marks the worst-of-N `√(2 ln N)` argument as a **model** and the disconnection *mechanism* as an **untraced hypothesis**. Also corrects the repo's "semi-discrete OT" wording: the problem is discrete-to-discrete; semi-discrete is its continuum limit
└── 10-mbo-auction-dynamics/  ← **approach B's derivation and provenance (2026-09-17).** §2 is a prior-art table READ FROM THE HELD PAPERS (all in `docs/papers/`, rows in its README): **Jacobs–Merkurjev–Esedoğlu 2018 §4.2 already computes equal-area minimal-perimeter tessellations of the FLAT 2-torus at N=64 and area-preserving flow at N=160**, Merriman–Ruuth 2007 run multiphase MBO on a CURVED torus (5 regions), grain growth runs at 10⁵ grains, and the implicit-Euler diffusion is the graph-MBO step of Garcia-Cardona et al. 2014 — so the earlier "flat, small N, outer problem" novelty sentence (docstring + report 08) was wrong on three of four clauses and is corrected in both places; the defensible claim is *combination, surface and scale*. Verified by two adversarial Fable reviews on 2026-09-17 (17 + 8 findings, all applied; the math survived both, the citations, the §7 mechanism and the §5.6 interpretation needed correction). **Every computed number is regenerated by its committed `check_numerics.py` → `numerics.yaml`** (seeded; ~5 min, `--quick` skips the 100×96 eigendecompositions) and the document carries a provenance block — see the gotcha below. Its Remark 6.1 records **why the standard inexact transportation-dual solver was used in place of JME's exact auction** (mesh forces a transportation problem; validated primitive existed; iterated scheme makes inexactness safe; cost bounded by G4) and the limit — "sufficed", never "better than the auction", which was not measured. Derivations: the backward-Euler step `(M+τK)⁻¹M` is an **exponential mixture of heat kernels** (`∫ τ⁻¹e^{-t/τ} e^{-tL} dt`), from which the resolvent kernel's interface constant is **½** (Gaussian 1/√π) — so `E_τ → ½ Σ_k Per(cell_k)`, i.e. the interface length counted once — and one step is mean-curvature flow over **τ/2, not τ**, checked on the 348×328 mesh against the **exact** continuum disc solution `y = 1 − xK₁(x)I₀(u)` (placement-averaged mesh loss at 0.91–0.97 of it and 0.48–0.54 of the Gaussian's; a single vertex-centred placement is the low tail of the granularity distribution, and comparing to the leading-order πτ alone is a cancellation — both misreadings were made and corrected in review). Balanced threshold = transportation dual (N=2 ⇒ Ruuth–Wetton's shifted level). **Esedoğlu–Otto dissipation does not apply as a theorem** to the implemented consistent-diffusion/lumped-assignment pairing: the assignment objective differs from the lumped energy's linearisation by the skew part of `D·A_τ` (relative size ~2×10⁻³; its symmetric part was PSD on every mesh tested, so the energy IS concave there) and the dual is inexact; the inequality that survives is gate G4; a lumped–lumped pairing `(D+τK)y = Dχ` would remove the skew term exactly (**not implemented**, and it would be isolating a small effect). Over-merge quantified: the cell's own diffused value at its centre is `1 − xK₁(x)`, x = R_cell/√τ, = 0.398 at the ρ=1 cap. Note the code's freeze constant `c_lo = √(R_cell/h)` was written with the Gaussian displacement; the resolvent's factor ½ shifts it by √2, inside the c=2/c=4 margin — noted, not changed
```
...
`main.pdf` outputs are tracked. **A math document that quotes any number
produced by running code must also carry a committed `check_numerics.py` and
its `numerics.yaml` output, plus a provenance block** — the experiments
folder's rule, extended to `docs/math/` on 2026-09-17
(`docs/math/AUTHORING_GUIDE.md` §8b; `10-mbo-auction-dynamics/` is the model).
Inline session Python and reviewer-supplied figures are not admissible sources.
...
8.18e-07** — converged to tolerance, genuinely equal-area).

**N=500 (`arm_mbo_20260822_120646`) extends the same finding**, on the *same*
5-level ladder so the meshes match exactly: all three gates pass on raw labels
(worst cell 1.0993% against a 1.4017% bar, 0 fragmented), Phase 1 **1,322 s**,
Phase 2 **412.1138** at iterate 10 of 20 (32,704 VPs, max violation 1.10e-06,
best mid-trajectory so **not censored**).

**The √N cross-check now has three points on the fixed V=114,144 mesh** — the
sharpest instrument available where no anchor exists, and the N=500 value was
recorded in the config *before* the run:

| | predicted | measured | dev |
|---|---|---|---|
| N=100 → N=400 | 368.8236 | 368.6603 | −0.044% |
| N=400 → N=500 | 412.1748 | **412.1138** | **−0.015%** |
| N=100 → N=500 | 412.3573 | 412.1138 | −0.059% |

**N=750 (`arm_mbo_20260822_182903`) needed a DIFFERENT ladder, and that is itself
a result.** The standard `100x96` base gives 12.8 v/cell and freeze ratio **0.964
— below 1.0**, so the coarse band cannot move; the config drops that rung and runs
`162x154 → 410x386` (V=158,260, 211 v/cell), every level at ratio ≥ 1.131. Result:
Phase 1 **3,614 s**, worst cell 1.2569% (bar 1.5165%), Phase 2 **504.9214** at
iterate 14/20 (47,088 VPs, max violation 4.68e-07) — and **0 fragmented at every
level**, the first high-N run to achieve that, where N=300 and N=500 each produced
one at level 0 that later levels had to heal. ⚠ It is on a different mesh, so it
...
profiles show **the IPOPT solver is 5.2% of the Phase 2 campaign at N=500 and
3.6% at N=750**, a *shrinking* share, so the solver is *not* the thing to
optimise. Phase 2 also scales superlinearly in variable points (0.250 → 0.270 →
0.389 s/VP; exponents 1.69 then 2.00), which makes it the binding constraint on N
— extrapolating gives **7–9 h of Phase 2 at N=1000**. See
`docs/plans/PUBLICATION_READINESS_PLAN.md` Phase 4 and the `timing_profile.yaml`
gotcha below.
...
- **Numbers in a document must be regenerable by a committed script — a lesson paid for on 2026-09-17.** `docs/math/10-mbo-auction-dynamics/` was first written with its §5.3/§5.6/§7.2 numbers computed in inline Python during the authoring session and two figures copied from an adversarial reviewer's scratch script; none of it could be re-run, and the §5.6 jittered-centre statistics depended on an unrecorded seed and parametrisation (two independent runs disagreed by ±0.03). The fix is structural, not a one-off: `docs/math/AUTHORING_GUIDE.md` §8b now requires `check_numerics.py` → committed `numerics.yaml` → `main.tex` with a provenance block for every math document that computes anything, exactly as `docs/experiments/README.md` requires `make_figures.py`/`data.yaml` for a report. When reviewing a document, the question to ask of every number is "which committed file regenerates this?" — if the answer is a session or a person, it is not auditable.
