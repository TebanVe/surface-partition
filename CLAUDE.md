...
★ = the best available at that N. All eight are `finalised=True`. The N=10 and
...
⚠ Phase 2 cost 7,326 s (N=400) and 8,829 s (N=500) against Phase 1's 920 s and
1,322 s, so **Phase 1 is now only 11–13% of end-to-end** — and the N=500 profile
shows **the IPOPT solver is just 5.2% of the Phase 2 campaign**, so the solver is
*not* the thing to optimise. See `docs/plans/PUBLICATION_READINESS_PLAN.md`
Phase 4 and the `timing_profile.yaml` gotcha below.
