...
├── torus_300part_rebased_ladder.yaml    # ⚠ **DO NOT RUN — under-corrected.** Would have re-based the N=300 ladder, but puts level 0 at 51 v/cell, still inside the dying regime (the runs/dies boundary is ~90 v/cell), so it would very probably change nothing. Lifting N=300's level 0 above it needs ~92 v/cell ⇒ V₀ ≈ 27,600 (base ~176×164). Kept as the record — see its header and the two ladder gotchas below
...
- **A floored line search is NORMAL convergence — do not read it as failure.** Every healthy Phase 1 level ends with the Armijo step at the backtracking floor `9.0949470177292824e-13` (= `pgd_step0 · pgd_backtrack_rho^40`) and the energy frozen for exactly `refine_patience` (30) iterations, at which point the plateau trigger fires. The validated N=100 deliverable `run_20260709_081548` does this on **four of its five levels**. An earlier `LINE SEARCH STALLED` warning fired on any floored step and therefore fired on every healthy level termination; two commits reasoned from it before it was checked against a known-good run. **What is diagnostic is *when* the floor is reached, not that it is.** The guard (`_STALL_EARLY_ITERS = 200` in `src/optimization/pgd_optimizer.py`) now warns `UNDER-RESOLVED LEVEL` only when the step floors inside the first 200 iterations. That threshold sits in an empty gap measured over all 130 levels in `results/`: levels floor either at iteration 18–100 or at 314+, never between. Firing means the level died before doing any work — check verts/cell first (everything below ~90 v/cell fires), then `lambda_penalty`. It does **not** detect the A2 soft-area pathology, where levels floored *late* and the collapse was still real. See `docs/experiments/06-subfloor-ladder/`.
- **A Phase 1 level either runs or dies, and the boundary is ~90 verts/cell.** Across every trace in `results/`, levels at 31/32/48/64/83 v/cell reach the backtracking floor at iteration **18–47** and stop; levels at 92/96 reach it at 1,141 or never in 30,000. A level below the boundary does *no* work — so the cheap equalization phase never happens there and the first level that runs starts from a badly imbalanced field. **Do not confuse this with the ~250–300 v/cell gate floor** of `winner_take_all_partition_gap.md` §4b (the resolution at which a level can itself clear the 5% gate); they differ by ~3× in mesh budget at N=1000. Caveat that stops it being a rule: a dying level 0 is not by itself fatal — N=200 reaches 0 imbalanced with a level 0 at 48 v/cell that dies at iteration 21, because its level 1 (124 v/cell) then runs 12,155 iterations. The statement consistent with every run is weaker: *some early, cheap level must run long enough to equalize before the expensive levels begin.*
- **N=300's ladder is exhausted — more iterations and more levels are both dead.** Reconstructed for free from the λ=11.5 control's saved trace iterates (`traces/*_internal_data.hdf5` hold `x`; rebuild the mesh with `TorusMeshProvider` and run the gates): seeded init 230/300 imbalanced / 44.02%; after L0 (32 v/cell) 234 / 45.57%; after L1 (83 v/cell) 234 / 49.26% — both coarse levels die and change nothing; **L2 (158 v/cell) does all the work and converges** (→ 10 imbalanced / 36.15%, flat over its last 1,000 iterations, so it did *not* stop early); L3 → 4 / 27.04%; L4 → 3 / 24.81%, i.e. decelerating returns of −9.11 and −2.23 points. Consistent with what the N=1000 analysis found by probing: the residual imbalance at high N is a property of the energy under the equal-*continuous*-mass constraint, not incomplete descent. **Untested**, and the one ladder question still open: whether giving N=300 an *earlier* level that actually runs (~92 v/cell ⇒ V₀ ≈ 27,600, base ~176×164) moves the converged point — report 06 shows that point is start-dependent. `parameters/torus_300part_rebased_ladder.yaml` does **not** test it (it lifts level 0 only to 51 v/cell, still in the dying regime). **Also note fragmentation is born mid-level-2** (iterations 2,500 and 4,500) while the imbalanced count falls 234 → 10 — balance is bought with disconnection on the *plain* energy too, the same mechanism report 05 measured for A2, milder. See `docs/reference/PHASE1_HIGHN_APPROACHES_ABCDE.md`.
